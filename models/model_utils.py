...
def classify_targets(classify, target=None):
    '''
    classify Type: torch.tensor with shape [batch,1,grid,grid]
    ''' 
    obj_scale = 1
    noobj_scale = 1
    bce_loss = nn.BCELoss()

    ByteTensor = torch.cuda.ByteTensor if classify.is_cuda else torch.ByteTensor
    # FloatTensor = torch.cuda.FloatTensor if classify.is_cuda else torch.FloatTensor 

    nB = classify.size(0) 
    nA = classify.size(1) # nA = 1
    nG = classify.size(2) 
    ...
    noobj_mask = Variable(1-pool(tconf), requires_grad=False).bool()

    # Loss 
    loss_conf_obj =  bce_loss(classify[obj_mask], tconf[obj_mask])
    loss_conf_noobj =  bce_loss(classify[noobj_mask], tconf[noobj_mask])
    loss = obj_scale * loss_conf_obj + noobj_scale * loss_conf_noobj
    return loss 
...
def nmsTest(prediction, conf_thres=0.5, nms_thres=0.4):
    """
    Removes detections with lower object confidence score than 'conf_thres' and performs
    Non-Maximum Suppression to further filter detections.
    Returns detections with shape:
